...
const CreateTranslationForm: FC<CreateTranslationFormProps> = ({ existingIds, fallback, onSubmit }) => {
  ...
  return (
    <>
      ...
      >
        {validCodes.filter((c) => !existingIds.includes(c)).map((code) => (
          <MenuItem key={`create-translation-id-${code}`} value={code} sx={{ py: 2 }} title={code}>
            <CountryFlag code={code.split('-').pop().toLowerCase()} width="30" />
            {code}
          </MenuItem>
        ))}
      </Select>
      {Object.keys(translation.translations).sort().map((key) => (
        <TextField
          key={`create-translation-form-input-${key}`}
          label={key}
          helperText={fallback[key]}
          value={translation.translations[key]}
          fullWidth
          sx={{ my: 2 }}
          onChange={(e) => {
            setTranslation({
      ...
      <Button
        ...
        disabled={!translation.id || !!Object.keys(translation.translations).find(k => !translation.translations[k])}
      >
        <FormattedMessage id="input.new.translation.submit" />
      </Button>
    </>
  )
}
